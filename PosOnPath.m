...
for j= 1:16
    ...
    xlim([4 8])
    ylim([4 8])
    n = [sin((deg2rad(angle(j)+90))) -cos((deg2rad(angle(j)+90)))];
...
%Also requered when lateral control is implemented on a real bicycle.
function [y,io] = SimulinkPosChooser(path,bike,i)
    ...
    while(j < length(path) && usnit >= 0)
        n = [sin((path(j,3)+pi/2)) -cos((path(j,3)+pi/2))];
